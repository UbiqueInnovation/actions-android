...
class AutoToolModel(OpenAIModel):
    ...
    def _prepare_completion_kwargs(self, *args, **kwargs):
        ...
        # smolagents passes ReAct stop tokens ("Observation:", "Calling tools:")
        # which can truncate a native tool-call response before the call is
        # emitted, leaving content with no parseable tool call. Drop them.
    ...
    def generate(self, *args, **kwargs):
        ...
        print(
            f"[raw] tool_calls={len(tcs) if tcs else 0} "
            f"finish={getattr(msg, 'finish_reason', None)} "
